...
const SECRET_CODE = "homework";

function AIAssistant() {
  const [messages, setMessages] = useState([
    {
      role: "ai",
      text: "Hello! I am SafeGuard AI, your personal safety assistant. How can I help you stay safe today?",
    },
  ]);
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", text: "Our AI assistant is temporarily unavailable due to high usage. Please try again in a few hours. For emergencies, press the SOS button immediately." },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSecretCode = (e) => {
    ...
    if (e.target.value.toLowerCase() === SECRET_CODE) {
      setDisguised(false);
      setSecretInput("");
    }
  };

  // DISGUISE MODE — looks like a notes app
  if (disguised) {
    return (
      <div className="min-h-screen bg-white p-6">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-2xl font-bold text-gray-700 mb-4">My Notes</h1>
          <textarea
            className="w-full h-64 border border-gray-300 rounded-lg p-4 text-gray-600 focus:outline-none"
            placeholder="Write your notes here..."
            value={secretInput}
            onChange={handleSecretCode}
          />
          <p className="text-gray-300 text-xs mt-2">Type your secret code to return</p>
        </div>
      </div>
    );
  }
      ...
      <div className="px-6 py-2 flex gap-2 overflow-x-auto">
        {QUICK_ACTIONS.map((action, index) => (
          <button
            ...
            className="bg-white border border-pink-300 text-pink-600 text-xs px-3 py-2 rounded-full whitespace-nowrap hover:bg-pink-50 transition"
          >
            {action}
          </button>
        ))}
